...
def get_rays(poses, intrinsics, H, W, N=-1, error_map=None, patch_size=1, mask_size=None, mask=None, modify_matrix=None):
    ...
    if N > 0:
        ...
        if mask!=None:
            if inds == None:
                inds = torch.randint(1, H*W, size=[N], device=device) # may duplicate
                inds = inds.expand([B, N])

            i = torch.gather(i, -1, inds) -0.5
            j = torch.gather(j, -1, inds) -0.5
            i = torch.flatten(i)
            j = torch.flatten(j)
            coords = torch.stack((i,j),dim=1) 

            id_mat = torch.stack([get_mask(W,mask_size,coords[:,0],coords[:,1])], dim=0)
            id_mat = torch.flatten(id_mat)

            id_mat=id_mat.long()
            boolen_mat = mask[id_mat] 
            if modify_matrix != None:
                modify_mat = modify_matrix[id_mat]
            boolen_mat = boolen_mat.expand([B, N])

            i = i.expand([B, len(i)])+0.5
            j = j.expand([B, len(j)])+0.5

            results['mask'] = 1 - boolen_mat
            if modify_matrix != None:
                results['modify'] = modify_mat


        if patch_size <= 1 and mask == None:
...
class Trainer(object):
    def __init__(self, 
        ...
        if self.opt.patch_size > 1:
            import lpips
            self.criterion_lpips = lpips.LPIPS(net=self.opt.net).to(self.device)
    ...
    def train_step(self, data):
        ...
        # MSE loss and modify loss
        if data['mask']!= None:
            mask = data['mask']
            mask = mask.unsqueeze(-1)
            loss = self.criterion(pred_rgb*mask, gt_rgb*mask).mean(-1) # [B, N, 3] --> [B, N]
            if self.opt.modify:
                modify = data['modify']
                modify = modify.unsqueeze(-1)
                loss = loss + self.opt.lambda1 * self.criterion(pred_rgb*modify*mask, gt_rgb*modify*mask).mean(-1)
        ...
        if self.opt.patch_size > 1:
            ...
            if data['mask'] != None:
                ...
                loss = loss + self.opt.lambda2 * self.criterion_lpips(pred_rgb*mask, gt_rgb*mask)
            else:
                loss = loss + self.opt.lambda2 * self.criterion_lpips(pred_rgb, gt_rgb)
        ...
        with torch.no_grad():
            ...
            for data in loader:    
                ...
                if self.local_rank == 0:
                    ...
                    # save image
                    save_path = os.path.join(self.workspace, 'result', f'{name}_{self.local_step:04d}_rgb.png')
                    save_path_depth = os.path.join(self.workspace, 'result', f'{name}_{self.local_step:04d}_depth.png')
...
# todo 这里是有问题的，但向上取整除法会报cuda assert bug？
def get_mask(W, mask_size, i, j):
    mask_row = j // mask_size
    mask_col = i // mask_size
    # (W + mask_size + 1) / mask_size
    mask_number = mask_row *(math.ceil(W/mask_size)) + mask_col
    return mask_number

# todo 生成ij后将ij代入mask_matrix剔除为1的ij